...
Discord-AI-Bot/
  src/
    ...
    games/                # 遊戲相關功能（按遊戲分類）
      hsr/                # 崩壞：星穹鐵道
        commands.ts       # /honkai-star-rail 指令
        service.ts        # MiHoMo API 服務
        cardCache.ts      # 卡片快取管理
        cardGenerator.ts  # 角色卡片生成器（呼叫 Python）
        generate_card.py  # Python 卡片生成腳本
      zzz/                # 絕區零
        commands.ts       # /zenless-zone-zero 指令
    db/
      index.ts            # 資料庫模組入口
    ...
    ai/
      gemini.ts           # Gemini AI 服務（兌換碼搜尋）
  pyproject.toml          # Python 依賴設定（uv）
...
1. 在 `src/games/<遊戲>/` 或 `src/bot/commands/` 建立新檔案
2. 匯出 `data` 和 `execute`
...
1. 在 `src/games/` 建立新的遊戲目錄（如 `src/games/genshin/`）
2. 建立 `commands.ts`（指令）和 `service.ts`（API 服務）
3. 在 `src/bot/commands/index.ts` 註冊指令
4. 更新 `src/db/storage.ts` 新增 UID 操作函數
5. 更新 `UidData` 介面
